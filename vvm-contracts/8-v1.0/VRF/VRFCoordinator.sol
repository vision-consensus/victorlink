...
contract VRFCoordinator is VRF, VRFRequestIDBase, Ownable {
    ...
    IBlockHashStore internal blockHashStore;
    ...
    constructor(
        ...
        blockHashStore = IBlockHashStore(_blockHashStore);
    }
    ...
    function registerProvingKey(
        ...
        bytes calldata _publicProvingKey,
        bytes32 _jobID
    ) external onlyOwner {
        //bytes32 keyHash = hashOfKey(_publicProvingKey);
        require(_publicProvingKey.length == 64, "length != 64 bytes, P.X||P.Y");
     ...
     * @param _feePaid The amount of payment given (specified in WVS)
    ...
    ) external onlyVictorMid checkCallbackAddress(_callbackAddress) {
        (bytes32 keyHash, uint256 consumerSeed) = abi.decode(
            _data,
            (bytes32, uint256)
        );
        ...
        permittedFunctionsForLINK(_data)
    {
        ...
        require(
            serviceAgreements[keyHash].vRFOracle != address(0),
            "please use a registered keyHash"
        );
    ...
    function randomnessRequest(
        ...
        uint256 _feePaid,
        address _sender
    ) internal sufficientWVS(_feePaid, _keyHash) {
    ...
    function hashOfKeyBytes(bytes memory _publicKey)
        public
        pure
        returns (bytes32)
    {
     ...
     */
    modifier sufficientWVS(uint256 _feePaid, bytes32 _keyHash) {
        require(
    ...
    modifier permittedFunctionsForLINK(bytes memory _data) {
        ...
        require(
            funcSelector == this.vrfRequest.selector,
