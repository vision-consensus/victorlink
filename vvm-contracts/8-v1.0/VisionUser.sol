...
import "../utils/IAggregator.sol";
import "../utils/librarys.sol";
import "./VictorlinkClientOracle.sol";
 ...
 */
contract Aggregator is IAggregator, VictorlinkClientOracle, Ownable {
        ...
        returns (bytes32)
    {
        ...
        for (uint256 i = 0; i < oracles.length; i++) {
            request = buildVictorlinkRequest(
                jobIds[i],
                address(this),
                this.victorlinkCallback.selector
            );
            requestId = sendVictorlinkRequestTo(
                oracles[i],
                request,
                oraclePayment
            );
        ...
        onlyOwner
    {
        ...
        require(
            victorMid.transferFrom(address(this), _recipient, _amount),
    ...
    function destroy() external onlyOwner {
        transferLINK(owner(), victorMid.balanceOf(address(this)));
    ...
    modifier validateAnswerRequirements(
        ...
        require(_oracles.length <= MAX_ORACLE_COUNT, "more than 45 oracles");
        require(
            _oracles.length >= _minimumResponses,
            "oracles larger than responses"
        );
        require(
            _oracles.length == _jobIds.length,
            "oracles larger than jobIDs"
        );
        _;
    }
