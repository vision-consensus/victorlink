...
import "./IAggregatorValidator.sol";
 ...
 */
contract FluxAggregator is IAggregator, IAggregatorV3, Ownable {
     ...
     * reserve of RESERVE_ROUNDS * oracleCount() WVS earmarked for payment to
     * oracles. (Of course, this doesn't prevent the contract from running out of
     * funds without the owner's intervention.)
     */
    uint256 public override constant version = 3;
     ...
     * @param _wvs The address of the WVS token
     * @param _paymentAmount The amount paid of WVS paid to each oracle per submission, in gwei (units of 1e-9 WVS)
    ...
    /**
     * @notice recalculate the amount of WVS available for payouts
     */
    function updateAvailableFunds() public {
        Funds memory funds = recordedFunds;

        uint256 nowAvailable = wvsToken.balanceOf(address(this)).sub(
            funds.allocated
        );
     ...
     * instead which includes better verification information.
     */
    function getAnswer(uint256 _roundId) public override view returns (int256) {
     ...
     * instead which includes better verification information.
     */
    function getTimestamp(uint256 _roundId) public override view returns (uint256) {
        if (validRoundId(_roundId)) {
    ...
    /**
     * @notice query the available amount of WVS for an oracle to withdraw
     */
    ...
    /**
     * @notice transfers the oracle's WVS to another address. Can only be called
     * by the oracle's admin.
     * @param _oracle is the oracle whose WVS is transferred
     * @param _recipient is the address to send the WVS to
     * @param _amount is the amount of WVS to send
     */
    function withdrawPayment(
        ...
        // Safe to downcast _amount because the total amount of WVS is less than 2^128.
        ...
        assert(wvsToken.transfer(_recipient, uint256(amount)));
    }

    /**
     * @notice transfers the owner's WVS to another address
     * @param _recipient is the address to send the WVS to
     * @param _amount is the amount of WVS to send
     */
        ...
        onlyOwner
    {
        ...
        require(
            wvsToken.transfer(_recipient, _amount),
    ...
    /**
     * @notice called through WVS's transferAndCall to update available funds
